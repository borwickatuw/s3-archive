...
class TestCreateTarGz:
    def test_round_trip(self, s3_client):
        ...
        create_tar_gz(s3_client, "src-bucket", "src/", "dest-bucket", "archive.tar.gz")

        # Re-extract and verify byte-for-byte.
        members = extract(
            s3_client, "dest-bucket", "archive.tar.gz", "dest-bucket", "out/", "tar.gz"
        )
    ...
    def test_dry_run_uploads_nothing(self, s3_client):
        ...
        create_tar_gz(
            s3_client, "src-bucket", "src/", "dest-bucket", "archive.tar.gz", dry_run=True
        )
    ...
    def test_empty_source_emits_warning_and_skips_upload(self, s3_client):
        ...
        create_tar_gz(s3_client, "src-bucket", "empty/", "dest-bucket", "archive.tar.gz")
        assert _list_keys(s3_client, "dest-bucket", "") == []
...
class TestCreateZip:
    def test_round_trip(self, s3_client):
        ...
        create_zip(s3_client, "src-bucket", "src/", "dest-bucket", "archive.zip")

        members = extract(s3_client, "dest-bucket", "archive.zip", "dest-bucket", "out/", "zip")
        assert set(members) == set(source)
        for rel, expected in source.items():
            assert _body(s3_client, "dest-bucket", "out/" + rel) == expected
    ...
    def test_dry_run_uploads_nothing(self, s3_client):
        _put_source(s3_client, "src-bucket", "src/", {"a.txt": b"alpha"})
        create_zip(s3_client, "src-bucket", "src/", "dest-bucket", "archive.zip", dry_run=True)
        assert _list_keys(s3_client, "dest-bucket", "") == []
...
class TestCreateDispatch:
    def test_tar_gz(self, s3_client):
        _put_source(s3_client, "src-bucket", "src/", {"a.txt": b"alpha"})
        create(s3_client, "src-bucket", "src/", "dest-bucket", "archive.tar.gz", "tar.gz")
        assert "archive.tar.gz" in _list_keys(s3_client, "dest-bucket", "")

    def test_zip(self, s3_client):
        _put_source(s3_client, "src-bucket", "src/", {"a.txt": b"alpha"})
        create(s3_client, "src-bucket", "src/", "dest-bucket", "archive.zip", "zip")
