...
**`.tar.gz`, `.tar.bz2`, and multi-block `.tar.xz` / multi-frame
`.tar.zst`.**
...
*often* single-block/frame, so this is a real, unavoidable gap for those
...
3. **Tier C handling.** Fail-fast message (recommended) vs. a weak
   re-read-from-front fallback (works but re-reads >1 GB, breaking the
   ≤1 GB promise)?
